...
private void GoFate()
        ...
        public List<FateData> MyFilter(List<FateData> List)
        {
            ...
            {
              if (f.Icon.ToString() == "Boss" && f.Progress < 15 || f.Id == 498)
                    Logging.Write("Skipping Boss  Fate {0} which Progress is lower than 15%", f.Name);
                else
                {
                    ReturnList.Add(f);
		...
		public  async Task<bool>  getFates()
		{
            ...
            if (currentfate == null) { return false; } else {
                return true; }
        ...
        public static FateData IsFateActive(uint id)
		{
            ...
            if (fateArray.Length > 0) 
               { return fateArray[0]; } else { return null; }
		}
